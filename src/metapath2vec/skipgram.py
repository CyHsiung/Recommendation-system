...
def build_model(BATCH_SIZE,VOCAB_SIZE,EMBED_SIZE,NUM_SAMPLED):
    ...
    with tf.name_scope('data'):
        center_node = tf.placeholder(tf.int64, shape=[BATCH_SIZE], name='center_node')
        context_node = tf.placeholder(tf.int64, shape=[BATCH_SIZE, 1], name='context_node')
        negative_samples = (tf.placeholder(tf.int64, shape=[NUM_SAMPLED], name='negative_samples'),
    ...
    with tf.name_scope('embedding_matrix'):
        ...
        print('embed_matrix', tf.shape(embed_matrix))
